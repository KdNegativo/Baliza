...
function BrakeVisual() {
  ...
  return (
    <svg className="service-visual brake-visual" viewBox="0 0 360 220" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      <defs>
        <radialGradient id="brakeDisc" cx="38%" cy="32%" r="70%">
          <stop offset="0%" stopColor="#f8fbf6" />
          <stop offset="42%" stopColor="#b8c1b8" />
          <stop offset="100%" stopColor="#58615b" />
        </radialGradient>
        <radialGradient id="brakeDiscFace" cx="44%" cy="42%" r="66%">
          <stop offset="0%" stopColor="#dfe5df" />
          <stop offset="58%" stopColor="#8e9a90" />
          <stop offset="100%" stopColor="#29332d" />
        </radialGradient>
        <radialGradient id="brakeHub" cx="40%" cy="35%" r="62%">
          <stop offset="0%" stopColor="#f2f6f0" />
          <stop offset="64%" stopColor="#a3aca4" />
          <stop offset="100%" stopColor="#343d37" />
        </radialGradient>
        <linearGradient id="caliperGradient" x1="0" x2="1" y1="0" y2="1">
          <stop offset="0%" stopColor="#8ced7d" />
          <stop offset="46%" stopColor="#22b85d" />
          <stop offset="100%" stopColor="#0b2517" />
        </linearGradient>
        <linearGradient id="caliperEdge" x1="0" x2="1">
          <stop offset="0%" stopColor="#193d24" />
          <stop offset="100%" stopColor="#ddffc2" />
        </linearGradient>
        <clipPath id="brakeRotorClip">
          <circle cx="132" cy="108" r="58" />
        </clipPath>
        <radialGradient id="brakePadGlow" cx="50%" cy="50%" r="65%">
          <stop offset="0%" stopColor="#eaffef" />
          <stop offset="100%" stopColor="#87e9a5" />
        </radialGradient>
      </defs>
      <path className="brake-shadow" d="M58 170C112 197 236 198 292 165" />
      <g className="brake-disc-svg">
        <circle className="rotor-shadow" cx="137" cy="113" r="70" />
        <circle className="rotor-outer" cx="132" cy="108" r="68" />
        <circle className="rotor-face" cx="132" cy="108" r="58" />
        <circle className="rotor-track" cx="132" cy="108" r="43" />
        <g clipPath="url(#brakeRotorClip)">
          {slots.map((angle) => (
            <rect
              className="rotor-vent"
              key={angle}
              x="127"
              y="55"
              width="10"
              height="27"
              rx="5"
              transform={`rotate(${angle} 132 108)`}
            />
          ))}
        </g>
        {holes.map((hole) => (
          <circle className="disc-hole" key={`${hole.cx}-${hole.cy}`} cx={hole.cx} cy={hole.cy} r="4.2" />
        ))}
        {bolts.map((bolt) => (
          <circle className="disc-bolt" key={`${bolt.cx}-${bolt.cy}`} cx={bolt.cx} cy={bolt.cy} r="3.7" />
        ))}
        <circle className="disc-hub" cx="132" cy="108" r="21" />
        <circle className="disc-center" cx="132" cy="108" r="10" />
      </g>
      <g className="brake-caliper-svg">
        <path className="caliper-shadow" d="M198 73H263C283 73 295 87 295 105V121C295 139 283 153 263 153H198L207 132H256C264 132 270 126 270 118V108C270 100 264 94 256 94H207Z" />
        <path className="caliper-shell" d="M195 70H262C282 70 294 85 294 104V122C294 141 282 156 262 156H195L205 132H255C263 132 269 126 269 118V108C269 100 263 94 255 94H205Z" />
        <path className="caliper-highlight" d="M210 80H259C274 80 283 90 283 104" />
        <path className="caliper-window" d="M219 98H253C260 98 264 103 264 110V117C264 124 260 129 253 129H219Z" />
        <rect className="brake-pad-svg pad-inner" x="188" y="94" width="13" height="34" rx="5" />
        <rect className="brake-pad-svg pad-outer" x="249" y="98" width="12" height="28" rx="5" />
        <path className="caliper-bridge-line" d="M211 88H263M208 145H263" />
        <circle className="caliper-bolt" cx="276" cy="94" r="3.8" />
        <circle className="caliper-bolt" cx="276" cy="134" r="3.8" />
        <circle className="caliper-bolt" cx="211" cy="83" r="3.2" />
      </g>
      <path className="brake-energy" d="M91 184H260" />
      <text className="visual-label brake-label disc-label" x="76" y="199">
        disco
      </text>
      <text className="visual-label brake-label caliper-label" x="244" y="199">
        pinça
      </text>
    </svg>
  )
...
function StickyCta() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const readingServices = services
        ? window.scrollY + window.innerHeight > services.offsetTop + 120 &&
          window.scrollY < services.offsetTop + services.offsetHeight - 80
        : false

      setVisible(window.scrollY > 600)
      setBlocked(nearContact || readingServices)
    }
